 ...
 * compare the raw type:/types: text directly against the SCIP symbol's
 * display_name, consulted by the CondType branch when the signature isn't
 * structured enough to build a Type.t). All SCIP-specific logic - including
 * this fallback's string matching - lives in this module, exactly like a
 * real Pro engine module would only expose an install-style entry point and
 * keep its internals out of the core engine files. *)
